...
func resolveOnce(ctx context.Context, source, canonical *url.URL, identifier map[string]any, proxy *proxyConfig, snapshot *probeData, options Options) (Response, error) {
	...
	transport, requestID, err := newTLSTransport(ctx, canonical.String(), proxy)
	if err != nil {
...
func transitionTask(transport *tlsTransport, identifier map[string]any, task task, args, nextArgs map[string]any, token, referrer string) (completionData, contentData, error) {
	if token != "" {
