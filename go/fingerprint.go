...
const (
	collectorEndpoint = "https://obseu.bizseasky.com/ct"
	// cheqFingerprint is the stable browser telemetry template expected by the CHEQ collector.
	cheqFingerprint = "W1siZWYiLDY5OTddLFsiYWJuY2giLDEwXSxbLTExLCJ7XCJ0XCI6XCJcIixcIm1cIjpbXCJkZXNjcmlwdGlvblwiXX0iXSxbLTI5LCItIl0sWy0zMSwiZmFs" +
...
// collectorURL adds fresh request and session identifiers to the fingerprint template.
func collectorURL(canonical string) (string, error) {
	now := time.Now()
	randomValue, err := randomDigits(97)
	if err != nil {
		return "", err
	}
	duid, err := collectorID(now.Unix())
	if err != nil {
		return "", err
	}
	suid, err := collectorID(now.Unix())
	if err != nil {
		return "", err
	}
	tuid, err := collectorID(now.Unix())
	if err != nil {
		return "", err
	}
	sid, err := collectorID(now.UnixMilli())
	if err != nil {
		return "", err
	}
	offset, err := cryptorand.Int(cryptorand.Reader, big.NewInt(501))
	if err != nil {
		return "", err
	}
	values := [][2]string{
		{"id", "14473"}, {"url", canonical}, {"sf", "0"}, {"tpi", ""}, {"ch", "cheq4ppc"},
		{"uvid", ""}, {"tsf", "0"}, {"tsfmi", ""}, {"tsfu", ""}, {"cb", strconv.FormatInt(now.UnixMilli(), 10)},
		{"hl", "2"}, {"op", "0"}, {"ag", "444981139"}, {"rand", randomValue}, {"fs", "756x441"},
		{"fst", "756x441"}, {"np", "windows"}, {"nv", "google inc."}, {"ref", ""}, {"ss", "800x600"},
		{"nc", "0"}, {"at", ""}, {"di", cheqFingerprint}, {"dep", "0"}, {"pre", "0"},
		{"sdd", ""}, {"cri", "qR3HM3UpbF"}, {"pto", strconv.FormatInt(400+offset.Int64(), 10)}, {"ver", "66"}, {"gac", "-"},
		{"mei", ""}, {"ap", ""}, {"fe", "1"}, {"duid", duid}, {"suid", suid},
		{"tuid", tuid}, {"sid", sid}, {"fbc", "-"}, {"gtm", "WyJvcGVuUGFnZSJd"}, {"it", "64,420,109"},
		{"fbcl", "-"}, {"gacl", "-"}, {"gacsd", "-"}, {"rtic", "-"}, {"rtict", "-"},
		{"bgc", "-"}, {"spa", "1"}, {"urid", "0"}, {"ab", ""}, {"sck", "-"}, {"io", "aGA2Oi1+aG02Og=="},
	}
	pairs := make([]string, len(values))
	for index, item := range values {
		pairs[index] = url.QueryEscape(item[0]) + "=" + url.QueryEscape(item[1])
	}
	return collectorEndpoint + "?" + strings.Join(pairs, "&"), nil
}

func collectorID(timestamp int64) (string, error) {
	token, err := randomToken(16)
	if err != nil {
		return "", err
	}
	return fmt.Sprintf("1.%d.%s", timestamp, token), nil
}
