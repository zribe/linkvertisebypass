...
public static class LinkvertiseClient
{
    ...
    private static async Task<BypassResponse> ResolveOnceAsync(Uri source, string canonical, JsonObject identifier, ProxyConnection? proxy, JsonObject? snapshot, BypassOptions options, CancellationToken token)
    {
        ...
        string requestId;
        try { (transport, requestId) = await TlsTransport.CreateAsync(canonical, proxy); }
        ...
        await using (transport)
        {
            ...
            if (current is null)
            {
                ...
                try
                {
                    bootstrap = await transport.BootstrapAsync(Request("getLinkByIdentifier", MetadataQuery, new { identifier = identifier.DeepClone() }), Request("getContent", ContentQuery, new { identifier = identifier.DeepClone(), task_args = Args(requestId, userId, sessionId, canonical) }), canonical);
                }
                catch (Exception error) { throw new RetryableException(error.Message, error); }
            ...
            for (var round = 0; round < 10; round++)
            {
                ...
                if (wait is not null)
                {
                    var (completion, next) = await TransitionAsync(transport, identifier, wait, Args(requestId, userId, sessionId, canonical), Args(requestId, userId, sessionId, canonical), "", canonical);
                ...
                if (ad is not null)
                {
                    ...
                    if (ad["ads"] is JsonArray { Count: > 0 } ads && ads[0] is JsonObject offer)
                    {
                        ...
                        await transport.SendEventsAsync(Text(taboola?["available_event_url"]), Text(taboola?["visible_event_url"]));
                        if (options.TaskDelay > TimeSpan.Zero) await Task.Delay(options.TaskDelay, token);
                    }
                    var transition = await TransitionAsync(transport, identifier, ad, Args(requestId, userId, sessionId, canonical, completionToken), Args(requestId, userId, sessionId, canonical), completionToken, canonical);
    ...
    private static async Task<(JsonObject Completion, JsonObject Content)> TransitionAsync(TlsTransport transport, JsonObject identifier, JsonObject task, JsonObject args, JsonObject nextArgs, string completionToken, string referrer)
    {
        if (completionToken.Length > 0) args["completion_token"] = completionToken;
        var raw = await transport.BatchAsync([
    ...
    private static async Task<(ProxyConnection Proxy, JsonObject Snapshot, bool Fresh)> ProbeAsync(ProxyConnection proxy, JsonObject identifier, string canonical, TimeSpan timeout, CancellationToken token)
    {
        try
        {
            ...
            request.Headers.UserAgent.ParseAdd(TlsUserAgent);
    ...
    // This identity stays fixed because it must match the embedded CHEQ browser fingerprint.
    internal const string TlsUserAgent = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/146.0.0.0 Safari/537.36";
