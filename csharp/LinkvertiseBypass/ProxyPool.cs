...
internal sealed record ProxyConnection(ProxyProvider Provider, Uri Server, string Username, string Password, string Country, string Lifetime, string Identity)
{
    ...
    {
        if (Username.Length == 0) return Server.ToString();
        var authority = Server.IsDefaultPort ? Server.Host : $"{Server.Host}:{Server.Port}";
        return $"{Server.Scheme}://{Uri.EscapeDataString(Username)}:{Uri.EscapeDataString(Password)}@{authority}";
    }
